...
class MailInstance
{
    ...
    {
        if (Config::getConfig('mail_type') !== 'api') {
            return (new Email())->send($mailto, $subject, $content, $fromname, App::$debug ? 1 : 0);
        } else {
            $aes = new AESEncrypt(Api::getInstance()->secretKey);
            $data = Api::getInstance()->request("/api/mail/send", ["mailto" => $mailto, "subject" => $subject, "fromname" => $fromname, "content" => $aes->encryptWithOpenssl($content)]);
