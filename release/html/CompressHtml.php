...
namespace cleanphp\release\html;
class CompressHtml
{
    static function compress($file)
    {
        ...
        foreach ($chunks as $c) {
            if (strtolower(substr($c, 0, 19)) == '<!--<nocompress>-->') {
                ...
                $compress .= $c;
                continue;
            } elseif (strtolower(substr($c, 0, 7)) == '<script' && strpos($c, '//') && (str_contains($c, "\r") || str_contains($c, "\n"))) { // JS代码，包含“//”注释的，单行代码不处理
                $tmps = preg_split('/(\r|\n)/ms', $c, -1, PREG_SPLIT_NO_EMPTY);
                $c = '';
                foreach ($tmps as $tmp) {
                    if (str_contains($tmp, '//')) { // 对含有“//”的行做处理
                        if (str_starts_with(trim($tmp), '//')) { // 开头是“//”的就是注释
                            continue;
                        }
